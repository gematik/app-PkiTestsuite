...
public class PcapManager implements Closeable {
  ...
  static String generatePcapFilename(
    ...
    // the test case is parameterized
    ...
    return String.format(
        "%s/%s_%s%s.pcap",
        outputDirname, testInfo.getTestMethod().orElseThrow().getName(), timestampStr, postfix);
  }
  ...
  public static void assignDevicesForIpAddresses(
    ...
    try {
      for (final Interface device : service.interfaces()) {
